...
@Composable
fun AccountScreen(
    navController: NavController, mAccountViewModel: AccountViewModel = hiltViewModel()
) {

    val mUserDataState by mAccountViewModel.mUserData.collectAsStateWithLifecycle()
    var mUserDataResult by remember { mutableStateOf<UserResponse?>(null) }
    val resultDataState by mAccountViewModel.logoutState.collectAsStateWithLifecycle()


    mUserDataState.handle(
        onSuccess = { data ->
            mUserDataResult = data
        },
    )

    resultDataState.handle(onSuccess = {
    ...
    Scaffold(topBar = {
        ...
        Column(
            ...
            PanelSettings(icon = painterResource(id = R.drawable.ic_sign_out_out_stroke_defaulti),
                ...
                isLogout = true,
                onClick = {
                    Log.d("checkproductishere", "AccountScreen: ")
